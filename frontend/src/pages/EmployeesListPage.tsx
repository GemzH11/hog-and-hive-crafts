...
export function EmployeesListPage() {
  ...
  async function deleteEmployeeAndReload(id: number) {
    ...
    try {
      ...
      if (!removed) setError("Employee not found (already deleted).")
      await loadEmployees()
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unknown error")
    } finally {
  ...
  return (
    <div className="rounded-lg border bg-white p-6">
      ...
      <form onSubmit={(e) => {
        ...
        <input disabled={loading} className="w-full rounded-md border px-3 py-2 text-sm disabled:bg-slate-100" placeholder="First name" value={firstName} onChange={(e) => setFirstName(e.target.value)} />
        <input disabled={loading} className="w-full rounded-md border px-3 py-2 text-sm disabled:bg-slate-100" placeholder="Last name" value={lastName} onChange={(e) => setLastName(e.target.value)} />
...
<button type="button" disabled={loading} onClick={() => {
                    // eslint-disable-next-line @typescript-eslint/no-floating-promises
                    deleteEmployeeAndReload(e.id)
                  }} className="text-red-700 hover:underline disabled:opacity-50">
                    Delete
                  </button>
                  <Link className="text-blue-700 hover:underline" to={`/employees/${e.id}/edit`}>
                    Edit
                  </Link>                  
                </td>
              </tr>
