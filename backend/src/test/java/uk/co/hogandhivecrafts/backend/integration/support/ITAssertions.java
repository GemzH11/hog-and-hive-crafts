 ...
 * differences in string formatting/precision between the database and JSON serialisation. Overloads
 * that accept file id lists are provided so callers can avoid traversing potentially-detached
...
public final class ITAssertions {
  ...
  /**
   * Assert that a {@link Pattern} entity matches the details in a {@link GetPatternByIdResponse}
   * DTO.
   * <p>
   * Timestamps are compared by {@link Instant} to avoid formatting/precision mismatches. File IDs
   * are supplied explicitly so this helper does not need to traverse a detached lazy collection.
   *
  ...
  /**
   * Assert that a {@link Pattern} entity matches a compact {@link GetAllPatternsItem} DTO used in
   * paged responses.
   * <p>
   * Timestamps are compared by {@link Instant} to avoid formatting/precision mismatches. File IDs
   * are supplied explicitly so this helper does not need to traverse a detached lazy collection.
   ...
   */
  public static void assertPatternItemEquals(Pattern expected, List<UUID> expectedFileIds,
    ...
    if (expected.getCreatedAt() != null) {
      ...
      Assertions.assertThat(actual.createdAt().toInstant())
    ...
    if (expected.getUpdatedAt() != null) {
      ...
      Assertions.assertThat(actual.updatedAt().toInstant())
                .isEqualTo(expected.getUpdatedAt().toInstant());
    } else {
      Assertions.assertThat(actual.updatedAt()).isNull();
    }
