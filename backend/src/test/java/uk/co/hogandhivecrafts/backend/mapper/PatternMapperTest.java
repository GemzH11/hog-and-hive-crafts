...
class PatternMapperTest {
  ...
  void toGetAllPatternsItem_mapsAllFields() {
    Pattern pattern = EntityTestData.buildDefaultPattern(UUID.randomUUID(),
  ...
  void toGetAllPatternsResponse_mapsAllFields() {
    Pattern pattern1 = EntityTestData.buildDefaultPattern(UUID.randomUUID(),
                                                          List.of(UUID.randomUUID(),
                                                                  UUID.randomUUID()),
                                                          UUID.randomUUID());
    Pattern pattern2 = EntityTestData.buildDefaultPattern(UUID.randomUUID(),
                                                          List.of(UUID.randomUUID(),
                                                                  UUID.randomUUID()),
                                                          UUID.randomUUID());
    ...
    Assertions.assertThat(response.patterns())
              .containsExactlyElementsOf(
                  page.getContent().stream().map(mapper::toGetAllPatternsItem).toList());
